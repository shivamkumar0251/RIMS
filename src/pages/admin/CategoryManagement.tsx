    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl animate-modal-pop flex flex-col h-[90vh]">
        <div className="p-5 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
          <div><p className="text-sm text-gray-500">For: <span className="font-semibold text-indigo-600">{targetName}</span></p></div>
...
const CategoryManagement: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([
    {
      id: 1, name: 'Setup', subcategories: [
        { id: 101, name: 'Kitchen' },
        { id: 102, name: 'Interior' },
        { id: 103, name: 'Crockery' },
        { id: 104, name: 'Electronics' },
        { id: 105, name: 'Furniture' },
        { id: 106, name: 'Lighting' },
        { id: 107, name: 'Appliances' },
        { id: 108, name: 'Decor' }
      ]
    },
    {
      id: 2, name: 'Products', subcategories: [
        { id: 201, name: 'Grocory' },
        { id: 202, name: 'Sweets' },
        { id: 203, name: 'Cleaning Supplies' },
        { id: 204, name: 'Sweet-Specific Ingredients' },
        { id: 205, name: 'Beverages' },
      ]
    }
  ]);
                ...
                {categories.map(category => (
                  <React.Fragment key={category.id}>
                    <tr className="bg-gray-50 hover:bg-gray-100 transition-colors">
                      ...
                      {/* Only allow edit subcategories, not delete top-level */}
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                        <button onClick={() => handleOpenSubCategoryModal('add', category)} className="text-green-600 hover:text-green-800" title="Add Subcategory">
                          <FiPlus size={18} />
                        </button>
                        {/* No delete for Setup/Products */}
                      </td>

                      {/* Only show Create Form for subcategories */}
                    ...
                    {category.subcategories.map(sub => (
                      <tr key={sub.id}>
                        ...
                        <td className="px-6 py-3 whitespace-nowrap text-sm font-medium space-x-3">
                          <button onClick={() => handleOpenSubCategoryModal('edit', category, sub)} className="text-yellow-600 hover:text-yellow-800">
                            <FiEdit size={18} />
                          </button>
                          <button onClick={() => handleDelete(sub.id, 'subcategory')} className="text-red-600 hover:text-red-800">
                            <FiTrash2 size={18} />
                          </button>
                        </td>
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            {sub.formFields?.length ? (
                              <>
                                <button onClick={() => handleOpenViewForm(sub.name, sub.formFields!)} className="..."><FiEye /> View</button>
                                <button onClick={() => handleOpenFormBuilder(sub, true)} className="..."><FiEdit /> Edit</button>
                              </>
                            ) : (
                              <button onClick={() => handleOpenFormBuilder(sub, true)} className="..."><FiFilePlus /> Create Form</button>
                            )}
                          </div>
                        </td>
                      </tr>
